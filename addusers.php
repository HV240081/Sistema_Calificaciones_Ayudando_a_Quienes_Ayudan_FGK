...
if ($id_usuario_modificar) {
  $sql = "SELECT U.ID_USUARIO, U.NOMBRE, U.APELLIDO, LOWER(CONCAT(U.NOMBRE, '_', U.APELLIDO)) AS USERNAME, U.CONTRASENA, R.ID_ROL, U.ESTADO 
          FROM USUARIO U 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  $role = $_POST['role'];
  $estado = isset($_POST['estado']) ? 1 : 0; // Nuevo campo para estado
  ...
  if ($stmt->fetchColumn() > 0) {
    ...
    // Actualizar usuario
    if ($id_usuario_post) {
      $query = "UPDATE USUARIO SET NOMBRE = ?, APELLIDO = ?, CONTRASENA = ?, ID_ROL = ?, ESTADO = ? WHERE ID_USUARIO = ?";
      $stmt = $pdo->prepare($query);
      if ($stmt->execute([$firstName, $lastName, $password, $role, $estado, $id_usuario_post])) {
      ...
      // Agregar nuevo usuario - Por defecto activo
      $query = "INSERT INTO USUARIO (NOMBRE, APELLIDO, CONTRASENA, TEMPORAL, ID_ROL, ESTADO) VALUES (?, ?, ?, 1, ?, 1)";
      $stmt = $pdo->prepare($query);
...
if (isset($_GET['toggle_estado'])) {
  $id_usuario_toggle = $_GET['toggle_estado'];
  
  // Obtener el estado actual
  $query = "SELECT ESTADO FROM USUARIO WHERE ID_USUARIO = ?";
  $stmt = $pdo->prepare($query);
  ...
  $stmt = $pdo->prepare($query);
  if ($stmt->execute([$nuevo_estado, $id_usuario_toggle])) {
    $accion = $nuevo_estado ? 'activado' : 'desactivado';
    $_SESSION['message'] = "¡Jurado $accion con éxito!";
    $_SESSION['alert-type'] = $nuevo_estado ? 'success' : 'warning';
  } else {
    $_SESSION['message'] = 'Error: ' . $stmt->errorInfo()[2];
    $_SESSION['alert-type'] = 'danger';
  }
...
$query = "SELECT U.ID_USUARIO, U.NOMBRE, U.APELLIDO, LOWER(CONCAT(U.NOMBRE, '_', U.APELLIDO)) AS USERNAME, U.CONTRASENA, R.ROL, U.ESTADO 
          FROM USUARIO U 
          JOIN ROL R ON U.ID_ROL = R.ID_ROL 
          ORDER BY U.ESTADO DESC, R.ROL, U.NOMBRE";
    ...
    <section class="section dashboard">
      <div class="row">
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-body">
              <h5 class="card-title"><?php echo $usuario ? 'Editar Usuario' : 'Agregar Usuario'; ?></h5>
                ...
                <div class="row mb-3">
                  <div class="col-md-6">
                    ...
                    <select id="role" name="role" class="form-select" required>
                      ...
                      <option value="3" <?php echo $usuario && $usuario['ID_ROL'] == 3 ? 'selected' : ''; ?>>Jurado principal</option>
                      <option value="2" <?php echo $usuario && $usuario['ID_ROL'] == 2 ? 'selected' : ''; ?>>Jurado</option>
                      <option value="1" <?php echo $usuario && $usuario['ID_ROL'] == 1 ? 'selected' : ''; ?>>Administrador</option>
                    </select>
                  </div>
                  <?php if ($usuario): ?>
                  <div class="col-md-6">
                    <label class="form-label">Estado del Jurado</label>
                    <div class="form-check form-switch mt-2">
                      <input class="form-check-input" type="checkbox" id="estado" name="estado" 
                             <?php echo $usuario && $usuario['ESTADO'] == 1 ? 'checked' : ''; ?>>
                      <label class="form-check-label" for="estado">
                        <?php echo $usuario && $usuario['ESTADO'] == 1 ? 'Activo (Puede votar)' : 'Inactivo (No puede votar)'; ?>
                      </label>
                    </div>
                  </div>
                  <?php endif; ?>
          ...
          <div class="card">
            <div class="card-body">
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                  <?php foreach ($usuarios as $usuario_row): ?>
                    <tr>
                      <td><?php echo htmlspecialchars($usuario_row['NOMBRE']); ?></td>
                      <td><?php echo htmlspecialchars($usuario_row['APELLIDO']); ?></td>
                      <td><?php echo htmlspecialchars($usuario_row['USERNAME']); ?></td>
                      <td><?php echo htmlspecialchars($usuario_row['CONTRASENA']) ?></td>
                      <td><?php echo htmlspecialchars($usuario_row['ROL']); ?></td>
                      <td>
                        <?php if (in_array($usuario_row['ROL'], ['EVALUADOR', 'JURADOPRINCIPAL'])): ?>
                          <?php if ($usuario_row['ESTADO'] == 1): ?>
                            <span class="badge bg-success badge-estado">Activo</span>
                          <?php else: ?>
                            <span class="badge bg-danger badge-estado">Inactivo</span>
                          <?php endif; ?>
                        <?php else: ?>
                          <span class="badge bg-secondary badge-estado">Administrador</span>
                        <?php endif; ?>
                      </td>
                      <td>
                        <a href="addusers.php?modificar=<?php echo htmlspecialchars($usuario_row['ID_USUARIO']); ?>"
                          class="btn btn-warning btn-sm"><i class='fas fa-edit'></i></a>
                        <?php if (in_array($usuario_row['ROL'], ['EVALUADOR', 'JURADOPRINCIPAL'])): ?>
                          <a href="addusers.php?toggle_estado=<?php echo htmlspecialchars($usuario_row['ID_USUARIO']); ?>"
                            class="btn btn-<?php echo $usuario_row['ESTADO'] == 1 ? 'warning' : 'success'; ?> btn-sm"
                            onclick="return confirm('¿Estás seguro de <?php echo $usuario_row['ESTADO'] == 1 ? 'desactivar' : 'activar'; ?> este jurado?')">
                            <i class='fas fa-<?php echo $usuario_row['ESTADO'] == 1 ? 'pause' : 'play'; ?>'></i>
                          </a>
                        <?php endif; ?>
                        <a href="addusers.php?delete=<?php echo htmlspecialchars($usuario_row['ID_USUARIO']); ?>"
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (estadoSwitch) {
      estadoSwitch.addEventListener('change', function() {
        ...
        label.textContent = this.checked ? 'Activo (Puede votar)' : 'Inactivo (No puede votar)';
      });
    }

    // Actualizar automáticamente el username cuando se escriben nombre y apellido
    ...
    function updateUsername() {
      ...
      if (firstName && lastName) {
        usernameInput.value = firstName.toLowerCase() + '_' + lastName.toLowerCase();
      }
    }
