...
// ===============================================
// OBTENER FONDOS ASIGNADOS (PIVOTEADOS) - MEJORADO
// ===============================================
try {
    // 1) Obtener proyectos con su ID_NFINAL y nota final calculada
    $proyectos_final = [];
    ...
    $proyectos = []; // key by project ID
    $maxFondos = 0;
    
    foreach ($proyectos_final as $id_proyecto => $proyecto_data) {
        $idn = $proyecto_data['ID_NFINAL'];
        $nombre = $proyecto_data['PROYECTO'];
        $nota = $proyecto_data['NOTA_FINAL'];

        $listaIds = ($idn && isset($asignaciones_map[$idn])) ? $asignaciones_map[$idn] : [];
